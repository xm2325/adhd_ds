...
def _payload(
    *,
    ...
    return {
        ...
        "scenario_impact": _records(scenario_impact),
...
def _html(payload: dict[str, Any], plotly_mode: str) -> str:
    template = "".join(path.read_text(encoding="utf-8") for path in sorted(ASSET_DIR.glob("dashboard_part_*.html")))
    css = (ASSET_DIR / "dashboard.css").read_text(encoding="utf-8")
    javascript = "\n".join(
        path.read_text(encoding="utf-8")
        for path in sorted(ASSET_DIR.glob("dashboard_*.js"))
    )
