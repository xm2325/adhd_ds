...
def write_casebook(question_catalog: pd.DataFrame, output_path: str | Path) -> None:
    ...
    lines = ['# Data scientist scenario casebook — synthetic ADHD service', '', '> Every numerical answer is generated from synthetic data. The casebook demonstrates how to answer workplace questions; it does not describe a real provider.', '', '## How to use this casebook', '', 'For each question, start with the decision, state the current evidence, explain the method, name the risk if wrong, and finish with the next action. Predictive, descriptive, scenario and causal claims are kept separate.', '']
    for category, group in question_catalog.groupby('category', sort=False):
        lines.extend([f"## {category.replace('_', ' ').title()}", ''])
        for _, row in group.iterrows():
            lines.extend([f"### {row['id']} — {row['question']}", '', f"**Asked by:** {row['stakeholder']}", '', f"**Why it is asked:** {row['why_asked']}", '', f"**Current synthetic answer:** {row['current_synthetic_answer']}", '', f"**How to solve it:** {row['method']}", '', f"**Inputs:** {row['required_inputs']}", '', f"**Output:** {row['deliverable']}", '', f"**Risk if wrong:** {row['risk_if_wrong']}", '', f"**Next action:** {row['next_action']}", ''])
